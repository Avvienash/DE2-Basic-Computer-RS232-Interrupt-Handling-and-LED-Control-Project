...
volatile int * MSB3;
volatile int * GREEN_LEDS;
volatile int * SW2;
volatile int * SEVEN;
volatile int * UART;
volatile int * RED_LEDS;
volatile int * MSB1;
volatile int * MSB2;
volatile int * Channel1;
volatile int * Channel2;
...
void interrupt_handler(void)
{
 ...
 if(ipending % 2 )// if ipending say irq 0 occured
 {
 *(MSB3 + 0x3 ) = 0x00;
 }

 if (ipending == 0x2 || ipending == 0x03)
 {
 char received_char = *(UART);

 redDisp = received_char<<8;
 if ((*(GREEN_LEDS)%5)==0 && *(SW2)==0)
 {
 redDisp = redDisp + 0x03;
 }
 *(RED_LEDS) = redDisp;
 int temp = (*(SEVEN)%(128));
 *(SEVEN) = *(SEVEN)>>7;
 *(SEVEN) = *(SEVEN) - (*(SEVEN)%(128)) + temp + (sevenSegment(received_char)<<21);
